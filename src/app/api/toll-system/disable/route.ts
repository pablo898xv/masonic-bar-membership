  ...
  try {
    ...
    if (!membership.tollSystemEnabled) {
      return NextResponse.json(
        { message: 'Card not enabled in toll system', status: 'INACTIVE' }
      )
    }
    ...
    const result = await tollSystem.disableCard(cardNumber, reason)
    
    if (!result.success) {
      return NextResponse.json({
        error: result.error || 'Failed to disable card in toll system',
    ...
    await prisma.membership.update({
      where: { id: membershipId },
      data: {
        tollSystemEnabled: false,
      }
    })
    
    return NextResponse.json({
      success: true,
      message: 'Card disabled in toll system',
      cardNumber,
      status: result.status,
      tollSystemConfigured: tollSystem.isConfigured(),
    })
  } catch (error) {
    console.error('Error disabling card in toll system:', error)
